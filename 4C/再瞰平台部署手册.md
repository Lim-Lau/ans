...
第二步：检查是否删除 
命令：java -version 提示 bash:java:未找到命令即可（若没有删除，可使用yum -y remove 命令删除）
...
![](http://image.kerbores.com/xsj/1584417003324.png)
修改 postgresql.conf，将监听修改为监听所有主机的请求
![](http://image.kerbores.com/xsj/1584417010459.png)
...
![](http://image.kerbores.com/xsj/1584427370490.png)
##清理脚本（默认保留最近7天的数据）
语句写法：
find 对应目录 -mtime +天数 -name "文件名" -exec rm -rf {} \;
find /crs/postgresql/bak -mtime+7 -name ".sql" -exec rm -rf {}\;
![](http://image.kerbores.com/xsj/1584428987365.png)
##### 定时任务
使用crontab配置定时任务，对pg数据进行备份和清理（默认凌晨1点进行数据备份和清理）
![](http://image.kerbores.com/xsj/1584429865947.png)
- 使用vi命令在/crs/sh目录下创建bakup.sh脚本
 ![](http://image.kerbores.com/xsj/1584428414492.png)
- 编写命令如下图，:wq保存退出；
![](http://image.kerbores.com/xsj/1584429073952.png)
- 给该脚本授权
命令：chmod 777 bakup.sh
![](http://image.kerbores.com/xsj/1584429220060.png)
- 使用 crontab -e 命令配置定时任务
![](http://image.kerbores.com/xsj/1584429302942.png)
输入命令：0 0 * * * /crs/sh/bakup.sh  后保存并退出 即可。
![](http://image.kerbores.com/xsj/1584429467139.png)
保存好的crontab文件目录位于 /var/spool/cron/（用什么用户创建的就会生成对应用户名的一个文件）
![](http://image.kerbores.com/xsj/1584429899522.png)
#### iServer10I安装、部署
(1)上传iServer安装包
...
保护模式：yes 改为 no   protected-mode no
...
![](http://image.kerbores.com/xsj/1584417375085.png)
打包成功如下图所示，找到打包文件路径（默认项目路径下的target目录下），获取jar即可。
![](http://image.kerbores.com/xsj/1584417369856.png)
方式二：使用本地maven命令进行打包
使用cmd窗口或者 git命令窗口，cd进入项目根目录；
![](http://image.kerbores.com/xsj/1584417364627.png)
执行mvn clean package -Dmaven.test.skip=true 命令；
![](http://image.kerbores.com/xsj/1584417348040.png)
![](http://image.kerbores.com/xsj/1584417356224.png)
备注：如果在使用mvn命令或者开发工具打包时，报错：No compiler is provided in this environment. Perhaps you are running on a JRE rather than a JDK?
![](http://image.kerbores.com/xsj/1584417338010.png)
错误原因：
...
![](http://image.kerbores.com/xsj/1584417317416.png)
b.设置缓存文件夹:npm config set cache "D:\vueProject\nodejs\node_cache"
c.设置全局模块存放路径:npm config set prefix "D:\vueProject\nodejs\node_global"
设置成功后，之后用命令npm install XXX -g安装以后模块就在D:\vueProject\nodejs\node_global里
d.基于 Node.js 安装cnpm（淘宝镜像）
npm install -g cnpm --registry=https://registry.npm.taobao.org
e.设置环境变量 （同其他软件设置环境变量）
3)安装vue
命令：cnpm install vue -g
![](http://image.kerbores.com/xsj/1584417308351.png)
4)安装vue命令行工具，即vue-cli脚手架
命令：cnpm install vue-cli -g
![](http://image.kerbores.com/xsj/1584417303448.png)
5)安装yarn
...
使用yarn命令打包
yarn && yarn run build
![](http://image.kerbores.com/xsj/1584417296158.png)
build成功后，默认路径在项目路径下的dist目录
![](http://image.kerbores.com/xsj/1584417289084.png)
(3)上传应用程序包至服务器后启动
在服务器 /crs/webapps目录下 执行rz命令，上传应用程序包
![](http://image.kerbores.com/xsj/1584417272360.png)
使用命令解压前端文件
![](http://image.kerbores.com/xsj/1584417266832.png)

(4)使用java命令启动crs-rest应用
命令： nohup java -jar crs-rest-1.0.0.jar --spring.config.location=/crs/config/application.yml &

![](http://image.kerbores.com/xsj/1584417256021.png)
(5)Nginx中配置 前端访问路径
![](http://image.kerbores.com/xsj/1584417239262.png)
#### License配置
...
![](http://image.kerbores.com/xsj/1584417445253.png)
可以通过修改配置文件中的license下path节点的值来指定路径
![](http://image.kerbores.com/xsj/1584417451518.png)
...
后端应用日志
默认路径：./logs/
![](http://image.kerbores.com/xsj/1584417553591.png)
查看日志(命令)
tail -500f ope-crs.log
tail -fn 100 ope-crs.log | grep   -- '关键字'
![](http://image.kerbores.com/xsj/1584417559822.png)

![](http://image.kerbores.com/xsj/1584417568472.png)
nginx日志
位置：/var/log/nginx/access.log
![](http://image.kerbores.com/xsj/1584417580394.png)
